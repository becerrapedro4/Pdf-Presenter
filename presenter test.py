...
from PyQt5.QtWidgets import (
    ...
    QAction # Added QAction for menu items
)
from PyQt5.QtCore import Qt, QSize, QTime, QTimer, QMimeData, QDateTime, QDate, QCoreApplication
...
class DocumentItem(QFrame):
    ...
    def __init__(self, doc_index, doc_name, thumbnail_pixmap, parent=None):
        ...
        self.thumbnail_label.setFixedSize(200, 150) # Fixed size for consistent thumbnails
...
class PDFPresenter(QMainWindow):
    ...
    def __init__(self):
        ...
        # self.setStyleSheet(self.get_light_style()) # Applied via load_theme_setting
        ...
        self.load_theme_setting() # Load and apply theme at startup
        ...
        # Connect signals for screen changes using the QApplication instance
        ...
        if app_instance:
            ...
            print("Warning: QApplication instance not found. Screen change signals may not be connected.")
    ...
    def load_theme_setting(self):
        """Loads the saved theme setting from a JSON file."""
        if os.path.exists(self._theme_setting_file):
            try:
                ...
                self._current_theme = "light" # Fallback to default
        self.apply_theme()

    def save_theme_setting(self):
        """Saves the current theme setting to a JSON file."""
    ...
    def toggle_theme(self):
        """Toggles between light and dark themes and saves the setting."""
    ...
    def get_light_style(self):
        ...
        return """
            ...
            QPushButton#btn_end_presentation { /* Specific style for end button */
                background-color: #cc0000; /* Red */
                color: white;
            }
            QPushButton#btn_end_presentation:hover {
            ...
            QComboBox {
                ...
                color: #333; /* Dark text for combobox */
            }
            QComboBox::drop-down {
            ...
            QComboBox::down-arrow {
                image: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZlJlYWR5ccllPAAAAGlJREFUeNpiYCAe/GBgYGYAYQYwXk3GgA0M/gPxgFgGiA/B+ICZA4iXgHgEiO8BfQPE/4BYC1AC8QkwJgDjBWD+B8QPIFxAFAHwAxCnAfEcIFaAYgHkAECzAbE2JGBgYADgAQB0QxnGngF+1gAAAABJRU5ErkJggg==); /* Black down arrow icon */
            }
            QComboBox QAbstractItemView {
                background-color: #f9f9fb; /* Light background for dropdown list */
                color: #333; /* Dark text for dropdown list items */
                selection-background-color: #7676ff; /* Blue highlight color */
    ...
    def get_dark_style(self):
        """
        Returns a CSS-like string for styling the application with a dark theme.
        """
        return """
            QMainWindow {
            ...
            QPushButton {
                background-color: #4a4a4a; /* Darker gray button background */
                border-radius: 8px;
                padding: 8px 16px;
                font-size: 14px;
                border: none;
            ...
            QComboBox {
                padding: 6px;
                border-radius: 6px;
                background-color: #3a3a3a; /* Dark background for combo box */
                border: 1px solid #555; /* Darker border */
                font-size: 14px;
                color: #e0e0e0; /* Light text for combobox */
            }
            QComboBox::drop-down {
                subcontrol-origin: padding;
                subcontrol-position: top right;
            ...
            QComboBox::down-arrow {
                image: url(data:image/png;base64,); /* White down arrow icon */
            }
            QComboBox QAbstractItemView {
                background-color: #3a3a3a; /* Dark background for dropdown list */
                color: #e0e0e0; /* Light text for dropdown list items */
            ...
            QProgressBar::chunk {
                background-color: #007bff; /* Blue progress chunk */
                border-radius: 5px;
            }
            QLabel {
    ...
    def init_initial_ui(self):
        ...
        label.setStyleSheet("font-size: 16px; color: #555;") # Keep color for this specific label as it's part of the initial view
    ...
    def create_menu_bar(self):
        ...
        toggle_theme_action = view_menu.addAction("Alternar Tema Oscuro/Claro")
    ...
    def update_document_selection_view(self):
        ...
        """
        # Clear existing widgets from the layout more robustly
    ...
    def render_page_for_selection(self, doc_index, page_in_doc_index, scale=0.5):
        ...
        try:
            ...
            if img.isNull():
                print("DEBUG: QImage.fromData devolvió una imagen nula. Posible error de formato.")
                try:
                    pix.save("debug_thumbnail_error.png")
                except Exception as e_save:
                    print(f"DEBUG: No se pudo guardar el pixmap para depuración: {e_save}")
                return None
    ...
    def setup_moderator_view(self):
        ...
        self.btn_end.setObjectName("btn_end_presentation") # Set object name for QSS targeting
    ...
    def render_page(self, page_number, scale=2):
        ...
        try:
            ...
            img = QImage.fromData(img_bytes)
            if img.isNull():
                print("DEBUG: QImage.fromData devolvió una imagen nula para presentación. Posible error de formato.")
                try:
                    pix.save("debug_presentation_error.png")
                except Exception as e_save:
                    print(f"DEBUG: No se pudo guardar el pixmap para depuración (presentación): {e_save}")
                return None
    ...
    def show_thumbnail_gallery(self):
        ...
        for page_num_in_doc in range(current_doc.page_count):
            ...
            thumbnail = QLabel()
            thumbnail.setFixedSize(350, 250) 
    ...
    def on_screen_added_or_removed(self):
        """
        Slot to handle screen added/removed signals. Updates the screen list and refreshes UI.
        """
        self.available_screens = QGuiApplication.screens()
        self.update_screen_selector()
        # Re-select the presentation screen if it's still available, otherwise default to first
        if self.selected_screen_index >= len(self.available_screens):
            self.selected_screen_index = 0 # Fallback
        
        # Ensure the screen_combo is updated with the correct index
        # Find the index of the previously selected screen by its data (original screen index)
        current_data = self.screen_combo.itemData(self.screen_combo.currentIndex())
        new_index = -1
        for i in range(self.screen_combo.count()):
            if self.screen_combo.itemData(i) == current_data:
                new_index = i
                break
        if new_index != -1:
            self.screen_combo.setCurrentIndex(new_index)
        elif len(self.available_screens) > 0:
            self.screen_combo.setCurrentIndex(0) # Select first if previous not found

        # If presentation is active, recreate it on the potentially new screen
        if self.presenter_window and self.presenter_window.isVisible():
            self.create_presentation_window()
    ...
    def update_screen_selector(self):
        ...
        Now uses 'Monitor #' format and is updated dynamically.
        """
        self.screen_combo.clear()
        for i, screen in enumerate(self.available_screens):
            # Store the original screen index as data, display "Monitor # - Name"
            self.screen_combo.addItem(f"Monitor {i+1} - {screen.name()}", i)
...
if __name__ == "__main__":
    try:
        # It's good practice to set a QApplication name for QSettings to work correctly
        # QCoreApplication.setApplicationName("PDFPresenter")
